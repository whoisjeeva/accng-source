...
class AccngModule(Module):
    def __init__(self):
        ...
        self.options.add("farming", "no", "Do some activity on the created account (yes, no)")
        ...
        self.description = f"Using MuMuPlayer to create Instagram account, {bold(yellow("required a mail server plugin."))} Public IP: {bold(red(self.public_ip))}"
    ...
    def run_runner(self, adb_id, proxy, account_count, emu_indexes):
        try:
            runner = Runner(
                ...
                farming=self.options.get("farming").strip() == "yes"
            )
            runner.start()
...
class Runner:
    def __init__(self, adb, adb_id, password, is_twofa, country, proxy, email_server, wait_time, stop_event, account_count, emu_indexes, farming):
        self.adb = adb
        self.adb_id = adb_id
    ...
    def wait_for_ocr_text(self, texts, multiply=False, throw_error=True):
    ...
    def start(self):
        ...
        for _ in range(self.account_count):
            try:
                ...
                r = requests.get("https://picsum.photos/500/500?seed=/seed/nature")
                ...
                self.click([118, 128])
                self.type_text_slow(f"{first_name} {last_name}")
                self.click([118, 194])
                self.type_text_slow(self.password)
                self.click([118, 306])


                self.wait_for_ocr_text(["Birthday", "birthday"])
                self.click([192, 650])
                self.wait_for_ocr_text(["Real Birthday", "real birthday"])
                self.click([189, 410])
                time.sleep(1)
                self.click([188, 661])

                self.wait_for_ocr_text(["Enter Your Age", "Enter your age"])
                self.click([139, 212])
                ...
                ocr_text = self.wait_for_ocr_text(["sync your", "Extra Security Step", "Logged Out", "code isn't valid", "photo"])
                if "Extra Security Step" in ocr_text:
                    log.info(f"Account suspended")
                    log.warn(f"!!!SYSTEM DETECTED!!!")
                    break
                elif "Logged Out" in ocr_text or "code isn't valid" in ocr_text:
                    continue

                if "sync your" in ocr_text:
                    self.click([191, 541])

                ocr_text = self.wait_for_ocr_text(["photo", "Extra Security Step"], multiply=True)
                if "Extra Security Step" in ocr_text:
                    log.info(f"Account suspended")
                    log.warn(f"!!!SYSTEM DETECTED!!!")
                    break
                ...
                self.click([192, 621])
                ocr_text = self.wait_for_ocr_text(["Gallery", "suspend"])
                ...
                self.click([90, 45])
                layout = self.wait_for_text("Other")
                self.click([80, 126])
                layout = self.wait_for_multi_text(["Gallery", "Pictures"])
                
                if "Pictures" not in layout:
                    self.click([26, 45])
                    layout = self.wait_for_text("Downloads")
                    self.click([96, 334])
                    layout = self.wait_for_text("Pictures")
                    coords = self.get_bounds(layout, "node", {"text": "Pictures"})
                    self.click(coords)
                    time.sleep(2)
                ...
                ocr_text = self.wait_for_ocr_text(["sync your", "Extra Security Step", "Logged Out", "code isn't valid", "Welcome", "suspend", "your story", "story"])
                if "Extra Security Step" in ocr_text:
                    log.info(f"Account suspended")
                    log.warn(f"!!!SYSTEM DETECTED!!!")
                    break
                ...
                if "Welcome" in ocr_text or "your story" in ocr_text or "story" in ocr_text:
                    ...
                    # while not self.stop_event.is_set():
                    #     subprocess.run([self.adb, "-s", self.adb_id, "pull", "/data/data/com.instagram.lite/files/PropertiesStore_v02", cookie_file_path], shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                    ...
                    # while not self.stop_event.is_set():
                    #     subprocess.run([self.adb, "-s", self.adb_id, "pull", "/data/data/com.instagram.lite/app_datastore/deviceid/device_id", device_id_path], shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
