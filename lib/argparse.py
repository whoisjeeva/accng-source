    ...
    def print_help(self):
        ...
        for argument in self.arguments:
            ...
            if argument["example"] is not None:
                s += "{} : {}\n".format(
                    format_space(argument["flag"] + other_flags + " " + argument["example"], self.argument_space_count),
                    argument["description"])
            else:
                s += "{} : {}\n".format(format_space(argument["flag"] + other_flags, self.argument_space_count),
                                        argument["description"])
        s += """
Copyright (c) 2019 mullaihub developers (http://suyambu.net/framework)
See the file 'LICENSE' for copying permission
"""
        print(s)
    ...
    def parse(self):
        ...
        for argument in self.arguments:
            ...
            for i, cmd_argument in enumerate(self.commandline_arguments):
                if (argument["flag"] == cmd_argument or cmd_argument in argument["other_flags"]) and argument[
                    "is_flag"]:
